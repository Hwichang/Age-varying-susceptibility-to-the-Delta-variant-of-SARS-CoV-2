...
#seiq_matrix = read.csv('initial_seiq_asym_0.04.csv')[,2:6]  # initial seiq asymptomatic 0.04
...
school_1 = as.matrix(contact_school$KOR)
school_1[1:4,] = (1/3)*school_1[1:4,]

school_2 =  as.matrix(contact_school$KOR)
school_2[1:3,] = (2/3)*school_2[1:3,]
school_2[4,] =  (skage_15/(skage_15+skage_16_18+skage_19))*(2/3) * school_2[4,]+(skage_16_18/(skage_15+skage_16_18+skage_19))*(1/3)*school_2[4,]

school_2.5 = (2/3)*as.matrix(contact_school$KOR)
school_2.5[1:4,] = (2/3)*school_2.5[1:4,]


contact_matrix = function(S){
  if(S>=286 & S<=328){
    res = as.matrix(contact_all$KOR) - school_1 - (0.1)*as.matrix(contact_others$KOR)  #Social distancing 1 / 1.5
  }else if(S>=329 & S<=342){
    res = as.matrix(contact_all$KOR) - school_2 - (0.08)*as.matrix(contact_work$KOR) - (0.23)*as.matrix(contact_others$KOR) #Social distancing 2
  }else{
    res = as.matrix(contact_all$KOR) - school_2.5 - (0.08)*as.matrix(contact_work$KOR) - (0.23)*as.matrix(contact_others$KOR) #Social distancing 2.5
  }
  return(res)
}
